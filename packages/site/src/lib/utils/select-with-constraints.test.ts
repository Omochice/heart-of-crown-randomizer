...
describe("selectWithConstraints", () => {
  ...
  it("should apply constraint that filters pool", () => {
    ...
    const result = selectWithConstraints(pool, [], new Set(), 3, [
      filterConstraint,
    ]);
  ...
  it("should respect both pinned cards and constraints", () => {
    ...
    const result = selectWithConstraints(pool, pinned, new Set(), 5, [
      filterConstraint,
    ]);
    expect(result).toHaveLength(5);
    expect(result.map((c) => c.id)).toContain(1);
  ...
  it("should respect excluded IDs with constraints", () => {
    ...
    const result = selectWithConstraints(pool, [], excludedIds, 5, [
      filterConstraint,
    ]);
    expect(result).toHaveLength(5);
    for (const card of result) {
  ...
  it("uses iterative selection when constraint has filterPoolForNextPick", () => {
    const dynamicConstraint: Constraint = {
      ...
      filterPoolForNextPick: (ctx: PickContext) =>
        ctx.pool.filter((c) => c.id <= 5),
    };

    const testPool = Array.from({ length: 20 }, (_, i) => makeCard(i + 1));
    const result = selectWithConstraints(testPool, [], new Set(), 3, [
      dynamicConstraint,
    ]);
  ...
  it("with link2GteLink0, all link-0 cards can appear across runs", () => {
    const link0Cards = Array.from({ length: 5 }, (_, i) =>
      makeCardWithLink(i + 1, 0),
    );
    const link2Cards = Array.from({ length: 8 }, (_, i) =>
      makeCardWithLink(i + 100, 2),
    );
    const link1Cards = Array.from({ length: 3 }, (_, i) =>
      makeCardWithLink(i + 200, 1),
    );
    const testPool = [...link0Cards, ...link2Cards, ...link1Cards];

    const seenLink0Ids = new Set<number>();
    for (let i = 0; i < 200; i++) {
      const result = selectWithConstraints(testPool, [], new Set(), 10, [
        link2GteLink0,
      ]);
      for (const card of result) {
