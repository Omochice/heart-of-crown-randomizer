...
describe("Randomizer Package Integration", () => {
  ...
  describe("filterByIds", () => {
    it("should filter out excluded cards by ID", () => {
      ...
      expect(filtered.every((card) => !excludedIds.includes(card.id))).toBe(
        true,
      );
      // Count how many cards were actually excluded (handles duplicates)
      const excludedCount = allCommons.filter((card) =>
  ...
  describe("select", () => {
    ...
    it("should select cards from the provided array", () => {
      ...
      expect(
        selected.every((card) => allCommons.some((c) => c.id === card.id)),
      ).toBe(true);
    });
  ...
  describe("drawRandomCards workflow", () => {
    it("should simulate the complete drawRandomCards workflow", () => {
      ...
      expect(randomCards.every((card) => !excludedIds.includes(card.id))).toBe(
        true,
      );
      expect(randomCards.every((card) => availableCommons.includes(card))).toBe(
        true,
      );
    });
  });
  ...
  describe("drawMissingCommons workflow", () => {
    it("should simulate the complete drawMissingCommons workflow", () => {
      ...
      const excludedIds = [...excludedCommons, ...selectedCommons].map(
        (c) => c.id,
      );
      const availableCommons = filterByIds(allCommons, excludedIds);
      ...
      expect(newCards.every((card) => !excludedIds.includes(card.id))).toBe(
        true,
      );
      expect(
        newCards.every(
          (card) => !selectedCommons.some((c) => c.id === card.id),
        ),
      ).toBe(true);
    });
