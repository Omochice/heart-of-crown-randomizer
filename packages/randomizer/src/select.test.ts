...
describe("select - Integration Tests", () => {
  it("should handle complex scenario with seed, exclusion, and required constraints", () => {
    ...
    const result = select(cards, 5, {
      seed: 42,
      constraints: {
        exclude: [(card) => card.type === "attack", (card) => card.cost > 5],
        require: requiredCards,
      },
    });
...
describe("select - Property-Based Tests", () => {
  ...
  test.prop([fc.array(fc.integer()), fc.nat(), fc.array(fc.integer())])(
  ...
  test.prop([fc.array(fc.integer({ min: 1, max: 100 })), fc.nat({ max: 50 })])(
  ...
  test.prop([fc.array(fc.integer()), fc.nat(), fc.integer()])(
